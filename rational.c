    ...
    struct Stack*restrict local_stack, struct Integer*numerator, struct Integer*denominator)
{
    ...
    integer_get_extended_gcd(local_stack, output_stack, &gcd_info, numerator, denominator);
...
{
    struct Rational*out = ALLOCATE(output_stack, struct Rational);
    out->numerator = integer_get_magnitude(output_stack, a->numerator);
    out->denominator = integer_copy(output_stack, a->denominator);
    return out;
}
...
{
    struct Rational*out = ALLOCATE(output_stack, struct Rational);
    out->numerator = integer_negate(output_stack, a->numerator);
    out->denominator = integer_copy(output_stack, a->denominator);
    return out;
}
...
struct Rational*rational_get_argument(struct Stack*output_stack, struct Rational*a)
{
    if (a->numerator->sign < 0)
    {
        struct Rational*out = ALLOCATE(output_stack, struct Rational);
        out->numerator = &one;
        out->denominator = integer_initialize(output_stack, 2, 1);
        return out;
    }
...
{
    void*local_stack_savepoint = local_stack->cursor;
    while (rational_compare(output_stack, local_stack, rational_get_magnitude(local_stack, delta),
        interval_size) > 0)
    {
    ...
    struct Stack*restrict local_stack, struct Rational*a, struct Rational*interval_size)
{
    ...
    while (true)
    {
        ...
        if (rational_compare(output_stack, local_stack, rational_get_magnitude(local_stack, delta),
            interval_size) <= 0)
        {
            out->min = rational_copy(output_stack, out->min);
            if (delta->numerator->value_count > 0)
    ...
    struct Rational*interval_size)
{
    ...
    int8_t ratio_magnitude_comparison = rational_compare(output_stack, local_stack,
        rational_get_magnitude(local_stack, ratio), &rational_one);
    ...
    else
    {
        ...
        out = rational_interval_negate(local_stack, output_stack,
            rational_estimate_arctangent(local_stack, output_stack,
                rational_get_reciprocal(local_stack, ratio), interval_size));
    }
    ...
    struct Stack*restrict local_stack, struct Rational*a, struct Rational*interval_size)
{
    ...
    out->min = float_reduce(output_stack, local_stack, out_get_min.significand, out_get_min.exponent);
    out->max = float_reduce(output_stack, local_stack,
    ...
    struct Stack*restrict local_stack, struct Rational*a, struct Rational*interval_size)
{
    ...
    {
        void*local_stack_savepoint = local_stack->cursor;
        struct FloatInterval*out = float_interval_negate(output_stack, local_stack,
            positive_rational_float_estimate(local_stack, output_stack,
                rational_negate(local_stack, a), interval_size));
        local_stack->cursor = local_stack_savepoint;
        return out;
    }
