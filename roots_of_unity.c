    ...
    struct Integer*degree)
{
    ...
    struct FloatInterval**argument_estimates = stack_slot_allocate(stack_a,
        (degree_size_t - 1) * sizeof(struct FloatInterval*), _Alignof(struct FloatInterval*));
    for (size_t i = 1; i < degree_size_t; ++i)
    {
        argument_estimates[i - 1] = number_float_argument_estimate(&permanent_pool_set, stack_a,
...
struct Number**get_roots_of_unity(struct Stack*stack_a, struct Stack*stack_b, struct Integer*degree)
{
    ...
    struct AlgebraicNumber**resolvents = stack_slot_allocate(stack_a,
    ...
    {
        resolvents[i] = STACK_SLOT_ALLOCATE(stack_a, struct AlgebraicNumber);
    ...
    struct Rational**resolvent_multiples_in_terms_of_degree_minus_first_roots =
        stack_slot_allocate(stack_a, resolvent_count_minus_one * degree_minus_one_size_t *
    ...
    for (size_t i = 0; i < resolvent_count_minus_one; ++i)
    {
        ...
        {
            struct AlgebraicNumber*resolvent_term =
    ...
    struct AlgebraicNumber*resolvent_power = STACK_SLOT_ALLOCATE(stack_a, struct AlgebraicNumber);
    ...
    struct Number**resolvent_product_values = stack_slot_allocate(stack_a,
    ...
    struct Number**resolvent_values = stack_slot_allocate(stack_a,
        degree_minus_one_size_t * sizeof(struct Number*), _Alignof(struct Number*));
    resolvent_values[0] = number_allocate(&permanent_pool_set);
    resolvent_values[0]->operation = 'r';
    resolvent_values[0]->value.numerator = pool_integer_initialize(&permanent_pool_set, 1, -1);
    resolvent_values[0]->value.denominator = pool_integer_initialize(&permanent_pool_set, 1, 1);
    resolvent_values[1] = number_exponentiate(&permanent_pool_set, stack_a, stack_b,
    ...
    for (size_t i = 1; i < degree_size_t; ++i)
    {
        out[i] = number_allocate(&permanent_pool_set);
        out[i]->operation = 'r';
        out[i]->value.numerator = pool_integer_initialize(&permanent_pool_set, 0, 0);
        out[i]->value.denominator = pool_integer_initialize(&permanent_pool_set, 1, 1);
        ...
        out[i] = number_rational_multiply(&permanent_pool_set, stack_a, stack_b, out[i],
