    ...
    struct Number*left_factor, struct Number*right_factor)
{
    ...
    out->operation = '*';
    out->left = number_copy(output_stack, left_factor);
    out->right = number_copy(output_stack, right_factor);
    struct NestedPolynomial*t =
        polynomial_allocate(local_stack, left_factor->minimal_polynomial->coefficient_count);
    for (size_t i = 0; i < left_factor->minimal_polynomial->coefficient_count; ++i)
    {
        t->coefficients[i] = polynomial_allocate(local_stack,
            left_factor->minimal_polynomial->coefficient_count - i);
        ...
        t->coefficients[i]->coefficients[degree] =
            left_factor->minimal_polynomial->coefficients[degree - i];
    }
        ...
        number_minimal_polynomial_from_annulling_polynomial(output_stack, local_stack,
            nested_polynomial_resultant(local_stack, output_stack, t,
                rational_polynomial_to_nested_polynomial(local_stack,
                    right_factor->minimal_polynomial)), out);
    ...
    struct Number*a, struct Number*b)
{
    ...
    struct Number*radicand_factor_a = number_exponentiate(local_stack, output_stack, a->left,
        &(struct Rational){info.b_over_gcd, &one});
    struct Number*radicand_factor_b = number_exponentiate(local_stack, output_stack, b->left,
    ...
    if (!integer_equals(a->right->value.denominator, b->right->value.denominator))
    {
        ...
        number_rational_argument_estimate(local_stack, output_stack, &a_argument_estimate, a,
            radicand_factor_argument_interval_size);
        struct RationalInterval b_argument_estimate;
        number_rational_argument_estimate(local_stack, output_stack, &b_argument_estimate, b,
            radicand_factor_argument_interval_size);
    ...
    struct Number*b)
{
    ...
    case '^':
    {
        if (b->operation == '^')
        {
            ...
            if (number_formal_compare(a, b) < 0)
            {
    ...
    case '*':
        ...
        {
            void*local_stack_savepoint = local_stack->cursor;
            struct Number*out = factor_consolidate(local_stack, output_stack, a->left, b);
            if (out)
            {
                out = number_multiply(output_stack, local_stack, out, a->right);
                local_stack->cursor = local_stack_savepoint;
                return out;
            }
            out = factor_consolidate(local_stack, output_stack, a->right, b);
            if (out)
            {
                out = number_multiply(output_stack, local_stack, out, a->left);
                local_stack->cursor = local_stack_savepoint;
                return out;
            }
            local_stack->cursor = local_stack_savepoint;
